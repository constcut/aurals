...
int GraphicMap::getAssociation(int x, int y)
{
    for (int i = elements.size()-1; i >= 0; --i)
    {
        int x2=0,y2=0;
        x2 = elements[i].w + elements[i].x;
        y2 = elements[i].h + elements[i].y;

        if ((x > elements[i].x && x < x2)
            && (y > elements[i].y && y < y2))
        {
            //emit initAssociation(elements[i].association);
...
void PianoQML::addOctave(int x, int y, int startingNote)
{
    for (int i = 0; i < 7; ++i)
    {
        ...
        //qDebug() << "Note shift "<<noteShift<<" "<<subShift;
...
{
    for (int i = 0; i < elements.size(); ++i)//learn foreach ...
    {
        if (elements[i].association == ass)
            return &elements[i];
    }
    return 0;
}
